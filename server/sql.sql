 				...
 				THEN -- если этот игрок не находится в другой игре
 				    if ((SELECT size from game WHERE game_id=id)>(SELECT count(*) FROM players WHERE game_id=id)) -- если у нас нет нужного кол-ва игроков
        			    THEN    
                            ...
                            INSERT INTO gameinformation(game_id, login, infoText) VALUES(id, log, "присоединился к игре.");
                            SELECT "cm is ok" as answer;
     				...
     				IF ((SELECT size FROM game WHERE game_id=id)=(SELECT count(*) from players WHERE game_id=id))
       					THEN 
                            ...
                            INSERT INTO gameinformation(game_id, login, infoText) VALUES(id, 1, "игра началась!");
                    ...
                    SELECT "Error! неправильный game_id " as err;        
    			end IF;
      
			else 
                SELECT "Error! у игрока уже есть id игры" as err;
			END IF;
    	ELSE 
            SELECT "Error! неправильный логин пароль" as err ;
   		END IF;
    ELSE 
        SELECT "Error!, id игры равен null (connectGame)"  as err;
    END IF;   
ELSE 
    SELECT "Error!, такого md игры не существует. Просмотри код еще разок."  as err;
END IF;
END//
DELIMITER ;
